...
private:    
    ...
    bool parseMessage(const String &msg, FullJointAngles &fja) {
        ...
        int8_t wristXIndex = msg.indexOf('x');
        int8_t gripperIndex = msg.indexOf('g');

        if (baseIndex == -1 || shoulderIndex == -1 || elbowIndex == -1|| wristIndex == -1 || wristXIndex == -1 || gripperIndex == -1) return false;
        ...
        fja.j4_wrist_angle = msg.substring(wristIndex + 1, wristXIndex).toInt() + SERIAL_ANGLE_OFFSET;
        fja.j5_wristX_angle = msg.substring(wristXIndex + 1, gripperIndex).toInt() + SERIAL_ANGLE_OFFSET;
        fja.j6_gripper_angle = msg.substring(gripperIndex + 1).toInt() + SERIAL_ANGLE_OFFSET;
